...
def tex_prisme_triangle_rectangle(enonce,baseTriangle,hauteurTriangle,hauteur,unite):
    enonce.append("\\begin{center}")
    enonce.append("\\psset{unit=0.5cm}")
    enonce.append("\\begin{pspicture}(-5,-3)(5,3.5)")
...
def Cylindre(parametre):
    question = "Calculer le volume du cylindre :"
    exo = []
    cor = []
    (rayon,hauteur,unite) = dimension_cylindre()
    tex_cylindre(exo,rayon,hauteur,unite)
    tex_cylindre(cor,rayon,hauteur,unite)
    cor.append("Volume = $\\pi \\times r \\times r \\times h$ \\newline")
    cor.append("Volume = $\\pi \\times %s \\times %s \\times %s$ \\newline" % (rayon,rayon,hauteur))
    cor.append("Volume = $\\boxed{\\unit[%s \\pi]{%s}}$ \\newline" % (rayon*rayon*hauteur,uniteVolume[unite]))
    cor.append("Volume $\\approx \\unit[%s]{%s}$" % (round(math.pi*rayon*rayon*hauteur,2),uniteVolume[unite]))
