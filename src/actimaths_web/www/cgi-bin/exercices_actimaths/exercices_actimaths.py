...
def creation_liste(liste_exercice):
    ...
    for i in range(len(liste_exercice)):
        (commandeTemp, parametre) = liste_exercice[i]
        commande = commandeTemp.split('.')
        ...
        while repetition:
            ...
            exec("from %s import %s" %(commande[0],commande[1]))
            (enonceTemp, correctionTemp, question ) = eval(commande[1])(parametre)
            (enonce, correction) = ("", "")
...
def copie_modele(tex, parametres, fiche, type, master, texte=''):
    ## chemin du modele
    source = join(dirname(__file__),'data', 'modeles', type, parametres['modele_%s' % type] , fiche + '_' + master  + '.tex' )
    ...
    if name == 'nt':
        environ['TEXINPUTS']= normpath(join(dirname(__file__),'data', 'packages'))
        tabvar = 'tabvar.tex'
    else:
        tabvar = normpath(join(dirname(__file__),'data', 'packages', 'tabvar.tex'))
